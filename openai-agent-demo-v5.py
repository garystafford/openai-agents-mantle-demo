...
def _print_answer(answer: QAAnswer, ctx: RunContext) -> None:
    print("\n--- final answer ---")
    print(answer.answer)
...
async def _run_once(prompt: str, ctx: RunContext) -> QAAnswer | None:
    streamed = Runner.run_streamed(qa_agent, prompt, context=ctx, max_turns=MAX_TURNS)
...
async def main(prompt: str) -> None:
    ...
    try:
        try:
            answer = await _run_once(prompt, ctx)
            ...
            try:
                ...
                answer = e2.guardrail_result.agent_output
        _print_answer(answer, ctx)
...
if __name__ == "__main__":
    # Try a prompt under 10 chars (e.g. "why?") to trip the input guardrail.
